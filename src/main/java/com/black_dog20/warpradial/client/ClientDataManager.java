...
public class ClientDataManager {
    ...
    public static boolean getPermissionOrIsOpOrSinglePlayer(Permission permission) {
        return Minecraft.getInstance().isSingleplayer() || hasLevelTo(permission) || getPermission(permission);
    }
    ...
    private static boolean hasLevelTo(Permission permission) {
        ...
        switch (permission) {
            case CAN_CREATE_SERVER_WARPS:
                return player.hasPermissionLevel(Config.SERVER_WARP_CREATE_PERMISSION_LEVEL.get());
            case CAN_DELETE_SERVER_WARPS:
                return player.hasPermissionLevel(Config.SERVER_WARP_DELETE_PERMISSION_LEVEL.get());
            case CAN_USE_MENU:
                return player.hasPermissionLevel(Config.USE_MENU_PERMISSION_LEVEL.get());
        }
