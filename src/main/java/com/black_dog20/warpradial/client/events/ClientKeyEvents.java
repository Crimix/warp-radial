...
public class ClientKeyEvents {
            ...
            if (keyIsDown && !keyWasDown) {
                ...
                if (!allowed && Config.INFORM_USER_OF_MISSING_PERMISSION.get()) {
                    Optional.ofNullable(Minecraft.getInstance().player)
                            .ifPresent(player -> player.sendStatusMessage(TranslationHelper.Translations.DENIED_MENU_USE.getComponent(), true));
                }
    ...
    private static boolean isAllowed() {
        if (Config.ONLY_PERMISSION_PLAYERS_CAN_USE_MENU.get()) {
            return ClientDataManager.getPermissionOrOpOrSP(PlayerPermissions::canUseMenu);
        } else {
            return true;
        }
    }
